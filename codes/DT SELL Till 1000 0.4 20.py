...
for row in range(len(parameter)):
    if parameter['run_dt'].iloc[row] == True:
        try:
            ...
            log = pd.DataFrame(columns =('Date/Day/Expiry.Day/Strike.Time/Target/CE.Strike/CE.Open/CE.Entry/Entry.Flag/Entry.Time/CE.High/CE.Low/CE.Exit/CE.SL.Flag/CE.SL.Time/CE.PNL/PE.Strike/PE.Open/PE.Entry/PE.Entry.Flag/PE.Entry.Time/PE.High/PE.Low/PE.Exit/PE.SL.Flag/PE.SL.Time/PE.PNL/Total PNL').split('/'))
