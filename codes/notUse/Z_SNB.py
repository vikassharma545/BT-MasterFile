...
for row in range(len(parameter)):
    if parameter['run_sut'].iloc[row] == True:
        try:
            ...
            print('Running Row :', row , index + ' SNB ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(straddle_sl) + ' ' + str(target) + ' OM-' + str(options_multiplier)) 
            ...
            log.to_csv(output_csv_path + index + ' SNB ' + str(day) + ' ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(straddle_sl) + ' ' + str(target)  + ' OM-' + str(options_multiplier) +'.CSV', index = False)
            t2 = datetime.datetime.now()
            print(t2-t1)
        except Exception as e:
            print(sys.exc_info()[-1].tb_lineno,type(e).__name__,asfd)
            msg = 'Error in row ' + str(row) + ": \n" + str(e) + "\n" + "Parameter : \n" + index + ' SNB ' + str(start_time).replace(':','')[:-2] + ' ' + str(end_time).replace(':','')[:-2] + ' ' + str(straddle_sl) + ' ' + str(target)  + ' OM-' + str(options_multiplier) 
            print(msg)
