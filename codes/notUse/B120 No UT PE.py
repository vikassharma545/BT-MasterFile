...
for row in range(len(parameter)):
    if parameter['run'].iloc[row] == True:
        try:
            ...
            find_strike = strangle_strike if options_multiplier > 0 else straddle_strike

            print(f"Running Row : {row}  {index}  B120 OM  {day}  {str(start_time).replace(':','')[:4]}  {str(end_time).replace(':','')[:4]}  SL-{sl}  {' OM-{0:.2f}'.format(options_multiplier)}")

            log = pd.DataFrame(columns =('Date/Day/Future/Expiry.Day/Start.Time/End.Time/Target/CE.Strike/CE.Open/CE.High/CE.Low/CE.Close/CE.SL.Price/CE.SL.Flag/CE.SL.Time/CE.PNL/PE.Strike/PE.Open/PE.High/PE.Low/PE.Close/PE.SL.Price/PE.SL.Flag/PE.SL.Time/PE.PNL/Total PNL').split('/'))
