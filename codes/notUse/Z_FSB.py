...
for row in range(len(parameter)):
    if parameter['run_dt'].iloc[row] == True:
        try:
            ...
            print('Running Row :', str(row), index + ' FSB ' + side + ' ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(sl) + ' ' + str(decay) + ' ' + '{0:.2f}'.format(options_multiplier))

            log = pd.DataFrame(columns =('Date/Day/Expiry.Day/Strike.Time/Target/FS.Strike/FS.Open/FS.Entry/FS.Entry.Flag/FS.Entry.Time/FS.High/FS.Low/FS.Exit/FS.SL.Flag/FS.SL.Time/Total PNL').split('/'))
            ...
            while from_date <= to_date:
                ...
                if ce_entry_flag and pe_entry_flag and ce_entry_time < pe_entry_time:
                    fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl = ce_scrip, ce_entry, ce_decay_price, ce_entry_flag, ce_entry_time, ce_high, ce_low, ce_exit, ce_sl_flag, ce_sl_time, ce_pnl
                elif ce_entry_flag and pe_entry_flag and pe_entry_time < ce_entry_time:
                    fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl = pe_scrip, pe_entry, pe_decay_price, pe_entry_flag, pe_entry_time, pe_high, pe_low, pe_exit, pe_sl_flag, pe_sl_time, pe_pnl
                elif ce_entry_flag and not pe_entry_flag:
                    fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl = ce_scrip, ce_entry, ce_decay_price, ce_entry_flag, ce_entry_time, ce_high, ce_low, ce_exit, ce_sl_flag, ce_sl_time, ce_pnl
                elif not ce_entry_flag and pe_entry_flag:
                    fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl = pe_scrip, pe_entry, pe_decay_price, pe_entry_flag, pe_entry_time, pe_high, pe_low, pe_exit, pe_sl_flag, pe_sl_time, pe_pnl
                else:
                    fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl = '', '', '', '', '', '', '', '', '', '', 0
                    
                print(from_date)
                log.loc[len(log.index)] = [str(from_date)[0:10], from_date.strftime("%A"), exp_day, strike_time.time(), target, fs_scrip, fs_entry, fs_decay_price, fs_entry_flag, fs_entry_time, fs_high, fs_low, fs_exit, fs_sl_flag, fs_sl_time, fs_pnl]
                from_date += datetime.timedelta(days=1)

            log.to_csv (output_csv_path + index + ' FSB ' + side + ' ' + str(day) + ' ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(sl) + ' ' + str(decay) + ' ' + '{0:.2f}'.format(options_multiplier) + '.csv', index = False)
            t2 = datetime.datetime.now()
            print(t2-t1)
        except Exception as e:
            msg = 'Error in row ' + str(row) + ": \n" + str(e) + "\n" + "Parameter : \n" + index + ' FSB ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(sl) + ' ' + str(decay) + ' ' + '{0:.2f}'.format(options_multiplier)  
            try:
                print(msg, from_date)
