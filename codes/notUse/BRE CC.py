...
def check_low(ftime,fdataframe,O):
    fdata = fdataframe.copy()
...
def low_SL(ftime, fdataframe, O , fscrip, sl_price, exit_price) : 
    
    if O <= fdataframe[fdataframe.date_time == ftime].close.iloc[0]:
        _,low_time = check_low(ftime,fdataframe,O) 
        
        return  low_time, fscrip, fdataframe, O, exit_price, sl_price
    
    else :
        data = options[(options.scrip.str.endswith(fscrip[-2:])) & (options.date_time>=ftime)]

        new_scrip = data[data['close']>O].sort_values(by=['date_time','close']).iloc[0].scrip

        new_data = options[(options.scrip == new_scrip)&(options.date_time >= ftime)]

        new_entry = new_data.close.iloc[0]
        
        new_exit = new_data.close.iloc[-1]
        
        sl_price = ((100 + re_sl)/100) * new_entry
        
        return  new_data.date_time.iloc[0], new_scrip, new_data, new_entry, new_exit, sl_price
...
for row in range(len(parameter)):
    if parameter['run'].iloc[row] == True:
        try:
            ...
            start_time = pd.to_datetime(parameter.loc[row,'start_time']).time()
            end_time = pd.to_datetime(parameter.loc[row,'end_time']).time()
            ...
            print(f'Running Row : {row} {index} BRE CC {str(start_time).replace(":", "")[:4]} {str(end_time).replace(":", "")[:4]} {sl} {re_sl} {options_multiplier:.2f}')
            
            u, s, t = [], [], []
            for w in range(0, re_entries+1):
                u.append(f'PL{w}')
                s.append(f'PL{w}.CE')
                t.append(f'PL{w}.PE')

            log = pd.DataFrame(columns =('Date/Day/Future/Expiry.Day/Time/Target/StrikeCE/CE Open/CE High/CE Low/CE Close/StrikePE/PE Open/PE High/PE Low/PE Close').split('/'))
            log = pd.DataFrame(columns = list(log.columns) + u + s + t + ['Total PNL'])
...
#                         vars()['pe_re_entry_time_' + str(i)] = ''
                    ...
                    fce_scrip, fce_data, fce_entry, fce_exit, fce_sl_price = ce_scrip, ce_data, ce_entry, ce_exit, ce_sl_price
                    ...
                    for i in range(1, re_entries+1):
                        if type(re_entry_time) == str:
                            ...
                            re_entry_time,ce_scrip, ce_data, ce_entry, ce_exit, ce_sl_price = low_SL(re_entry_time, ce_data, ce_entry ,ce_scrip, ce_sl_price, ce_exit)
                            if type(re_entry_time) == str:
                                break
                            else :
                                vars()['ce_sl_trigger_' + str(i)], vars()['ce_sl_time_' + str(i)], vars()['ce_pnl_' + str(i)] = high_SL(re_entry_time, ce_data, ce_entry, ce_exit, ce_sl_price)
                    ...
                    fpe_scrip, fpe_data, fpe_entry, fpe_exit, fpe_sl_price = pe_scrip, pe_data, pe_entry, pe_exit, pe_sl_price
                    #put re_entry
                    ...
                    for i in range(1, re_entries+1):
                        if type(re_entry_time) == str:
                            ...
                            re_entry_time,pe_scrip, pe_data, pe_entry, pe_exit, pe_sl_price = low_SL(re_entry_time, pe_data, pe_entry, pe_scrip, pe_sl_price, pe_exit)
                            if type(re_entry_time) == str:
                                break
                            else:
                                vars()['pe_sl_trigger_' + str(i)], vars()['pe_sl_time_' + str(i)], vars()['pe_pnl_' + str(i)] = high_SL(re_entry_time, pe_data, pe_entry, pe_exit, pe_sl_price)
                    ...
                    u, s, t = [], [], []
                    for w in range(0, re_entries+1):
                        u.append(round(vars()['pl_' + str(w)], 2))
                        s.append(round(vars()['ce_pnl_' + str(w)], 2))
                        t.append(round(vars()['pe_pnl_' + str(w)], 2))       

                    print(from_date)
                    log.loc[len(log.index)] = [str(from_date)[0:10] ,from_date.day_name() , future_price, exp_day, entry_reference_time.time(), target, fce_scrip[:-2], fce_entry, ce_high, ce_low, fce_exit, fpe_scrip[:-2], fpe_entry, pe_high, pe_low, fpe_exit] + u + s + t + [total_pl]
                    from_date += datetime.timedelta(days=1)
                except Exception as e:
                    msg = f'Error in Date {str(from_date)[0:10]}: \n{e}\nParameter: \n{index} BRE {str(start_time).replace(":","")[:-2]} {str(end_time).replace(":","")[:-2]} {sl} {re_sl} {options_multiplier:.2f}'
                    print(msg)
                    try:
            ...
            log.to_csv (output_csv_path + f'{index} BRE CC {day} {str(start_time).replace(":","")[:4]} {str(end_time).replace(":","")[:4]} {sl} {re_sl} {re_entries} {options_multiplier:.2f}.csv' , index = False)
