...
def OHLC_SL(fdata, entry_f, fsl):
    
    sl_price_f = ((100 - fsl)/100) * entry_f
    entry_f = entry_f + Cal_slipage(entry_f)
...
def OHLC_UT_SL(ftime, fdata, fsl):
    ...
    try:
        ...
        return entry_f, high_f, low_f, exit_f, sl_flag, sl_time, pnl
    except:
        return '', '', '', '', '','', 0
...
def find_my_strangle(start_dt):
    while start_dt < end_dt:
        try:
            ...
            return ce_scrip, pe_scrip, start_dt, ce_entry, pe_entry, future_price, target
        except:
...
def find_my_straddle(start_dt):
    while start_dt < end_dt:
        try:           
            ...
            return ce_scrip, pe_scrip, start_dt, ce_price, pe_price, future_price, "ATM"
        except:
            start_dt += datetime.timedelta(minutes = 1)
...
for row in range(len(parameter)):
    if parameter['run_b120'].iloc[row] == True:
        try:
            ...
            print(' '.join(('Running Row :', str(row), index, 'B120GB', str(day), str(start_time).replace(':','')[:4], str(end_time).replace(':','')[:4], 'SL-' + str(sl), ' OM-' + '{0:.2f}'.format(options_multiplier))))

            log = pd.DataFrame(columns =('Date/Day/Future/Expiry.Day/Time/Target/CE.Strike/CE.Open/CE.High/CE.Low/CE.Close/CE.SL.Flag/CE.SL.Time/CE.PNL/PE.Strike/PE.Open/PE.High/PE.Low/PE.Close/PE.SL.Flag/PE.SL.Time/PE.PNL/Total PNL').split('/'))
            ...
            while from_date <= to_date:
                try:
                    ...
                    if index in ['FINNIFTY', 'MIDCPNIFTY','BANKEX', 'SENSEX']:
                        gap = get_gap(options)
                    ...
                    if options_multiplier == 0:
                        ce_scrip, pe_scrip, start_dt, ce_entry, pe_entry, future_price, target = find_my_straddle(start_dt)
                    else:
                        ce_scrip, pe_scrip, start_dt, ce_entry, pe_entry, future_price, target = find_my_strangle(start_dt)
                    ...
                    ce_data = options[(options.scrip == ce_scrip) & (options['date_time'] > start_dt) & (options['date_time'] <= end_dt)]
                    ce_high, ce_low, ce_exit, ce_sl_trigger, ce_sl_time, ce_pnl = OHLC_SL(ce_data, ce_entry, sl)

                    pe_data = options[(options.scrip == pe_scrip)  & (options['date_time'] > start_dt) & (options['date_time'] <= end_dt)]
                    pe_high, pe_low, pe_exit, pe_sl_trigger, pe_sl_time, pe_pnl = OHLC_SL(pe_data, pe_entry, sl)
                    ...
                    print(from_date)
                    log.loc[len(log.index)] = [str(from_date)[0:10], from_date.day_name(), future_price, exp_day, entry_reference_time.time(), target, ce_scrip, ce_entry, ce_high, ce_low, ce_exit, ce_sl_trigger, ce_sl_time, ce_pnl, pe_scrip, pe_entry, pe_high, pe_low, pe_exit, pe_sl_trigger, pe_sl_time, pe_pnl, total_pnl]
                    from_date += datetime.timedelta(days=1)
                except Exception as e:
                    msg = 'Error in Date ' + str(from_date)[0:10] + ": \n" + str(e) + "\n" + "Parameter : \n" + index + ' B120GB' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' SL-' + str(sl) + ' OM-' + str(options_multiplier)  
                    print(msg)
                    try:
            ...
            log.to_csv(output_csv_path + index + ' B120GB ' + str(day) + ' ' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' ' + str(sl) + ' ' + '{0:.2f}'.format(options_multiplier) + '.csv', index = False)
            t2 = datetime.datetime.now()
            print(t2-t1)
        except Exception as e:
            msg = '*Error in row ' + str(row) + ": \n" + str(e) + "\n" + "Parameter : \n" + index + ' B120 BUY' + str(start_time).replace(':','')[:4] + ' ' + str(end_time).replace(':','')[:4] + ' SL-' + str(sl) + ' OM-' + str(options_multiplier) + '*'    
            print(msg)
            try:
